...
module.exports = {
  generateReport,
  generateCombineReport
}

function generateCombineReport(reports, outputFolder) {
  var fileName =  path.join(outputFolder, 'output.html');
  ...
  stream.once('open', function(fd) {
    ...
    var html = `
      <html>
          ...
          ${reports.map(report => `<iframe src="${report}" frameborder="0" scrolling="no" > </iframe>`).join('\n')}
...
/** @type { (runName: string, suite: Suite, deployments: Deployment[], eventLog: EventLogRecord[], kbStatus: any[], esStatus: any[], kbTaskManagerStatusList: Map<string, Map<string, TaskManagerStats[]>>) => string } */
function generateReport(runName, suite, deployments, eventLog, kbStatus, esStatus, kbTaskManagerStatusList, ruleStatus, reportFileName, outputFolder) {
  const { zone } = deployments[0]
  const reportName = `${runName}-${suite.id}`
  
  const content = readTemplate(reportFileName)
    ...
    .replace('["%RuleStatus%"]', JSON.stringify(ruleStatus))
  ...
  logger.log(`${new Date().toISOString()}: generated report ${filePath}`)
  return fileName
}
