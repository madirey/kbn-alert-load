...
async function createRule(kbUrl, ruleName, ruleType) {
  /** @type {any} */
  if (ruleType === 'query') {
    await retry(120, 5, `creating ${ruleType} rule`, async () => {
      await httpClient.post(`${kbUrl}/api/detection_engine/rules`, queryRule(ruleName, ruleName))
    })
    ...
    await retry(120, 5, `creating ${ruleType} rule`, async () =>
      await httpClient.post(`${kbUrl}/api/detection_engine/rules`, indicatorRule(ruleName, field))
    )
  } else if (ruleType === 'eql') {
    await retry(120, 5, `creating ${ruleType} rule`, async () =>
      await httpClient.post(`${kbUrl}/api/detection_engine/rules`, eqlRule(ruleName, ruleName))
    )
    ...
    await retry(120, 5, `creating ${ruleType} rule`, async () =>
      await httpClient.post(`${kbUrl}/api/detection_engine/rules`, thresholdRule(ruleName, thresholdField))
    )
  }
