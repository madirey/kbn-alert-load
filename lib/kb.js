...
async function createRule(kbUrl, ruleName, scenario, enabled=true) {
  ...
  if (scenario.ruleType === 'query') {
    ...
    for (let i=0; i < scenario.numAggFields; i++) {
      thresholdFields.push(`num_value_${i}`);
    }
    rule = thresholdRule(ruleName, thresholdFields, 10, enabled, scenario.alertInterval || '1m')
  }
