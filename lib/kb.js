...
async function findRuleStatuses(kbUrl, ruleIds) {
  const data = {
    ids: ruleIds
  }
  const response = await retry(10, 120, `getting rule statuses`, async () =>
      await httpClient.post(`${kbUrl}/api/detection_engine/rules/_find_statuses`, data)
  )
  return response.data || {}
}
