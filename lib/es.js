...
async function getEventCount(esUrl) {
  const response = await retry(10, 2, `getting event count`, async () =>
    await httpClient.get(`${esUrl}/test-index/_count`)
  );
...
async function deleteAllAlerts(esUrl) {
  try {
    const response = await httpClient.post(`${esUrl}/.alerts-security.alerts-default/_delete_by_query`, {
      query: {
        match_all: {},
      },
    });
...
async function createDetection(esUrl, ruleType, value = "ALERT", numValueGroup = [0], indicatorIndexSize = 0, indicatorFieldCount = 0 ) {
  const timestamp = new Date().toISOString()
  let data = ''
  const document = {
    ...baseDocument,
  ...
  for (let i=0; i < numValueGroup.length; i++) {
    document[`num_value_${i}`] = numValueGroup[i];
    // create random events (for higher cardinality and interleaving)
    randomDocument[`num_value_${i}`] = randomInteger(0, 999999999999);
  }

  if (ruleType === 'threshold') {
    // Creates 500 events for this group
    for (let i = 0; i < 500; i++) {
