...
/** @type { (esUrl: string, ruleType: string, firing: boolean) => Promise<any[]> } */
async function createTraffic(esUrl, count=1000) {
  const timestamp = new Date().toISOString()
  let data = ''
  ...
  for (let i = 0; i < count; i++ ) {
    const document = {
      ...
      "@timestamp": timestamp,
      "value": "abc" + Math.floor(Math.random() * 999999999999).toString(),
      "num_value": Math.floor(Math.random() * 999999999999),
      "indicator_value": "abc" + Math.floor(Math.random() * 999999999999).toString(),
      "event": {
        "category": "process"
      }
    }
  ...
  const response = await retry(10, 2, `creating traffic`, async () =>
      await httpClient.post(`${esUrl}/test-index/_bulk`, data)
  )
  return response.data || []
}
