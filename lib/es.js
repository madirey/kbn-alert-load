...
async function putILMPolicy(esUrl, ilmName, warmDuration=1, coldDuration=3, deleteDuration=10) {
  const data = {
    "policy": {
      "phases": {
        "hot": {
        ...
        "warm": {
          "min_age": `${warmDuration}m`,
          "actions": {}
        },
        "cold": {
          "min_age": `${coldDuration}m`,
          "actions": {}
        },
        "delete": {
          "min_age": `${deleteDuration}m`,
          "actions": {
            "delete": {}
          }
        }
...
/** @type { (esUrl: string, ilmName) => Promise<any[]> } */
async function putIndexTemplate(esUrl, ilmName) {
  const data = {
    "index_patterns": [
      "test-*"
    ],
...
/** @type { (esUrl: string, firing: boolean) => Promise<any[]> } */
async function createIndicator(esUrl, firing) {
  const timestamp = new Date().toISOString()
  const data = {
    "@timestamp": timestamp,
    "value": firing ? 'ALERT' : 'no alert'
  }

  const response = await retry(10, 2, `creating detections indicator`, async () =>
      await httpClient.post(`${esUrl}/test-index/_doc/`, data)
  )
  return response.data || []
}
