...
async function createDeployment({ config, stack, name, deploymentName, esSize, kbSize, tmMaxWorkers, tmPollInterval }) {
  ...
  args.push('--deployment-template', 'gcp-io-optimized')
  args.push('--generate-payload')
  // args.push('--file', './deploy_template.json');
  // args.push('--docker_image', 'docker.elastic.co/kibana/kibana:8.1.0-SNAPSHOT');

  // generate the payload to create the deployment
  // const payload = await ecctlRun(args)
  const payload = require('./deploy_template.json');
  updatePlan(payload, esSize, kbSize)
...
function updatePlan(payload, esSize, kbSize) {
  payload.resources.apm = null
  payload.resources.appsearch = null
  payload.resources.enterprise_search = null
  for (let i = 0; i < payload.resources.elasticsearch[0].plan.cluster_topology.length; i++) {
    if (payload.resources.elasticsearch[0].plan.cluster_topology[i].id === "hot_content") {
      payload.resources.elasticsearch[0].plan.cluster_topology[i].size.value = esSize * 1024
      payload.resources.elasticsearch[0].plan.cluster_topology[i].zone_count = 1
    }
    if (payload.resources.elasticsearch[0].plan.cluster_topology[i].id === "master") {
      if (esSize >= 64 * 6) {
        payload.resources.elasticsearch[0].plan.cluster_topology[i].size.value = 1 * 1024
      }
    }
  }
  payload.resources.kibana[0].plan.cluster_topology[0].size.value = kbSize * 1024
}