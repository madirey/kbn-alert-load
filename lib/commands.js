...
const { createAlert, createAction, createRule, updateRule, waitForRuleToRun, getKbStatus, getKbTaskManagerStatus, createDefaultIndex } = require('./kb')
const {
  getEventLog,
  getEsStatus,
  putClusterSettings,
...
async function run({ config, minutes, percentFiring }, [ suiteId ]) {
  ...
  const ruleUpdates = deployments.map(async (deployment, index) => {
    const rulePatchJSON = {}
  ...
  let trafficInterval = 1
  const traffic = setInterval(async () => {
    logger.log(`Firing traffic`)
    runIndicators(deployments, percentFiring)
    trafficInterval += 1
  }, 60 * 1000).unref()
  deployments.map(deployment => runTraffic(deployment, minutes * 60))
...
async function runIndicators(deployments) {
...
async function runTraffic(deployment, duration) {
  if (deployment.scenario.trafficRate !== undefined) {
    const file = 'ssh'
    const trafficGenerator = 'ubuntu@ec2-3-238-23-222.compute-1.amazonaws.com'
    const args = []
    const interval = 10
