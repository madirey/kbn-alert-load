...
const { createAlert, createAction, createRule, getKbStatus, getKbTaskManagerStatus, createDefaultIndex } = require('./kb')
const {
  getEventLog, getEsStatus, putClusterSettings, putILMPolicy, putIndexTemplate, createIndicatorIndex, createDetection} = require('./es')
...
const CONCURRENT_TRAFFIC = 100
const STATS_INTERVAL_MILLIS = 15 * 1000
const NO_TEARDOWN = false

/** @type { CommandHandler } */
async function run({ config, minutes, percentFiring }, [ suiteId ]) {
  if (suiteId == null) {
  ...
  // @ts-ignore
  // const deploymentPromises = scenarios.map((scenario, index) =>
  ...
  logger.log(`creating ilm and index template and seed fields in test index`)
  deployments.map(async deployment => {
    if (deployment.scenario.searchableSnapshot) {
      const ilmName = "hot-warm-cold"
      putClusterSettings(deployment.esUrl)
      putILMPolicy(deployment.esUrl, ilmName)
      putIndexTemplate(deployment.esUrl, ilmName)
    }
    await createDetection(deployment.esUrl, deployment.scenario.ruleType)
  })
  ...
  const queues = deployments.map(async deployment => {
    if (deployment.scenario.ruleType === undefined) {
      ...
      if (deployment.scenario.ruleType === 'indicator') {
        logger.log(`${deployment.name}: creating threat-indicator index with ${deployment.scenario.indicatorCount} size`)
        await createIndicatorIndex(deployment.esUrl, deployment.scenario.indicatorCount)
      }
      logger.log(`${deployment.name}: creating ${deployment.scenario.alerts} ${deployment.scenario.ruleType} rules`)
      const alertNames = arrayFrom(deployment.scenario.alerts, (i) => `${i}`.padStart(5, '0'))
      return await runQueue(alertNames, CONCURRENT_ALERT_CREATION, async (alertName, i) => {
        try {
          const data = await createRule(deployment.kbUrl, `${alertName}`, deployment.scenario.ruleType)
          return data
        } catch (err) {
          logger.log(`error creating rule ${deployment.scenario.ruleType}_${alertName} in ${deployment.scenario.name}, but continuing: ${err.message}`)
        }
      })
    }
  })
  ...
  const traffic = setInterval(async () => {
    runTraffic(deployments, percentFiring)
  }, 60 * 1000).unref()
...
async function lsv({ config }) {
  ...
  for (const { id, description, scenarios } of suites) {
    ...
    for (const scenario of scenarios) {
      ...
      if (scenario.indicatorCount != undefined) {
        logger.log(`${prefix2}indicatorCount:   ${scenario.indicatorCount}`)
      }
    }
    logger.log('')
  }
...
async function runTraffic(deployments) {
  const indicatorQueues = deployments.map(async deployment => {
    if (deployment.scenario.trafficCount !== undefined) {
      const alertNames = arrayFrom(deployment.scenario.alerts, (i) => `${i}`.padStart(5, '0'))
      const traffic = [...Array(deployment.scenario.trafficCount).keys()]
      return await runQueue(traffic, CONCURRENT_TRAFFIC, async (alertName, i) => {
        try {
