...
/** @type { CommandHandler } */
async function runrule({ config, minutes, percentFiring , trafficGenerator }, [ suiteId ]) {
  if (suiteId == null) {
    return logger.logErrorAndExit('suite id must passed as an parameter')
  }
  ...
  if (trafficGenerator === undefined){
    ...
    deployments.map(deployment => {
      if (deployment.scenario.trafficRate !== undefined) {
        runTraffic(trafficGenerator, deployment.esUrl, deployment.scenario.trafficRate, minutes * 60)
      }
    })
  }
