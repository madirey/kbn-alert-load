...
async function runIM({ configSequence, reportFileName, outputFolder, minutes}) {
  ...
  for (const suite of suites) {
    const reportName = await run({esUrl, kbUrl, reportFileName, suiteData: suite, scenario:1, minutes, outputFolder}, []);
...
async function runThreshold({ config, configSequence, reportFileName, outputFolder, minutes}) {
  const configSequenceData = require(path.join('..', configSequence))
  ...
  const reports = [];
  for (const suite of suites) {
    const reportName = await run({config, esUrl, kbUrl, reportFileName, suiteData: suite, scenario:1, minutes, outputFolder}, []);
    reports.push(reportName)
  }
...
/** @type { CommandHandler } */
async function run({ config, minutes, percentFiring, esUrl, kbUrl, scenario, trafficGenerator, trafficGeneratorId, reportFileName, suiteData, outputFolder }, [ suiteId ]) {
  ...
  deployments.map(async deployment => {
    logger.log(`Deleting existing rules and indices`)
    await deleteTestIndex(deployment.esUrl);
    await deleteIndicatorIndex(deployment.esUrl);
    await deleteAllAlerts(deployment.esUrl);
  ...
  const interval = setInterval(async () => {
    ...
    updateEventCount(deployments, eventCounts);
  }, STATS_INTERVAL_MILLIS).unref();

  // TODO: make configurable
  const nThresholdGroups = 150;
  ...
  const traffic = setInterval(async () => {
    ...
    logger.log('Starting threshold traffic')
    const detections = deployments.map(async (deployment) => {
      if (deployment.scenario.ruleType === 'threshold') {
        const numValues = [];
        for (let i=0; i < nThresholdGroups; i++) {
          const numValueGroup = [];
          for (let j=0; j < deployment.scenario.numAggFields; j++) {
            numValueGroup.push(Math.floor(Math.random() * 999999999999));
          }
          numValues.push(numValueGroup);
        }
        return await runQueue(numValues, 10, async (numValueGroup) => {
          const promises = [];
          return await createDetection(deployment.esUrl, 'threshold', 'ALERT', numValueGroup);
        });
      }
    });
    await Promise.all(detections);
  }, 45 * 1000).unref(); // every 45 seconds
  ...
  const reportName = generateReport(runName, suite, deployments, completeLog, eventCounts, kbStatusList, esStatusList, kbTaskManagerStatusList, ruleStatus, reportFileName, outputFolder)
  ...
  async function updateEventCount(deployments, eventCounts) {
    for (const deployment of deployments) {
      try {
        const count = await getEventCount(deployment.esUrl);
        const countStats = {
          count,
          timestamp: new Date().toISOString(),
        };
        eventCounts.push(countStats);
      } catch (err) {
        logger.log(`error getting events in test-index from ${deployment.scenario.name}: ${err}`)
      }
    }
  }
