...
const { splitX, delay, retry } = require('./utils')
...
async function waitForHealthyDeployment(config, id, name, wait = 1000 * 60 * 15, interval = 10000) {
  ...
  const info = await retry(10, 2, `getting deployment info`, async () =>
      await ecCommands.getDeployment({ config, name, id })
  )
  if (info.healthy) {
